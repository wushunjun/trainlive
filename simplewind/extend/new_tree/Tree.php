...
class Tree
{
     ...
     * @param array 2维数组，例如：
     * array(
     *      1 => array('id'=>'1','category_pid'=>0,'name'=>'一级栏目一'),
     *      2 => array('id'=>'2','category_pid'=>0,'name'=>'一级栏目二'),
     *      3 => array('id'=>'3','category_pid'=>1,'name'=>'二级栏目一'),
     *      4 => array('id'=>'4','category_pid'=>1,'name'=>'二级栏目二'),
     *      5 => array('id'=>'5','category_pid'=>2,'name'=>'二级栏目三'),
     *      6 => array('id'=>'6','category_pid'=>3,'name'=>'三级栏目一'),
     *      7 => array('id'=>'7','category_pid'=>3,'name'=>'三级栏目二')
     *      )
     * @return array
     */
    ...
    public function getParent($myId)
    {
        ...
        if (!isset($this->arr[$myId]))
            return false;
        $pid = $this->arr[$myId]['category_pid'];
        $pid = $this->arr[$pid]['category_pid'];
        if (is_array($this->arr)) {
            foreach ($this->arr as $id => $a) {
                if ($a['category_pid'] == $pid)
                    $newArr[$id] = $a;
            }
        }
        return $newArr;
    ...
    public function getTree($myId, $str, $sid = 0, $adds = '', $str_group = '')
    {
        ...
        if (is_array($child)) {
            ...
            foreach ($child as $key => $value) {
                ...
                $this->ret .= $nstr;
                $nbsp      = $this->nbsp;
                $this->getTree($parentId, $str, $sid, $adds . $k . $nbsp, $str_group);
                $number++;
            }
        }
        return $this->ret;
    ...
    public function getTreeArray($myId, $maxLevel = 0, $level = 1)
    {
        ...
        if (is_array($children)) {
            foreach ($children as $child) {
                $child['_level']           = $level;
                $returnArray[$child['category_id']] = $child;
                if ($maxLevel === 0 || ($maxLevel !== 0 && $maxLevel > $level)) {

                    $mLevel                                = $level + 1;
                    $returnArray[$child['category_id']]["children"] = $this->getTreeArray($child['category_id'], $maxLevel, $mLevel);
                }
    ...
    function getTreeViewMenu($myId, $effected_id = 'example', $str = "<span class='file'>\$name</span>", $str2 = "<span class='folder'>\$name</span>", $showlevel = 0, $ul_class = "", $li_class = "", $style = 'filetree ', $currentlevel = 1, $recursion = FALSE, $dropdown = 'hasChild')
    {
        ...
        foreach ($child as $id => $a) {
            ...
            @extract($a);
            if ($showlevel > 0 && is_array($this->getChild($a['category_id']))) {
            ...
            $recursion = FALSE;
            //判断是否为终极栏目
            if ($this->getChild($a['category_id'])) {
                eval("\$nstr = \"$str2\";");
                $this->str .= $nstr;
                if ($showlevel == 0 || ($showlevel > 0 && $showlevel > $currentlevel)) {
                    $this->getTreeViewMenu($a['category_id'], $effected_id, $str, $str2, $showlevel, $ul_class, $li_class, $style, $currentlevel + 1, TRUE);
                } elseif ($showlevel > 0 && $showlevel == $currentlevel) {
                    //$this->str .= $placeholder;
                }
            } else {
    ...
    public function createSubJson($myId, $str = '')
    {
        ...
        if (is_array($sub_cats))
            foreach ($sub_cats as $c) {
                $data[$n]['category_id'] = iconv(CHARSET, 'utf-8', $c['catid']);
